//! The swarm-tile crate acts as the Rust API for developing plugins for Swarm.
//!
//! To read more about Swarm plugins:
//! [https://swarm.dev/documentation/plugins](https://swarm.dev/documentation/plugins)
//!
//! ### Interesting things in this libary:
//! - The [`SwarmPlugin`] trait for implementing plugins combined with the
...
//! - The [`SwarmWorker`] trait for implementing background workers combined with the
...
//! [https://github.com/swarm-org/rust-plugin-example](https://github.com/swarm-org/rust-plugin-example)
//!
pub mod prelude;
pub mod shim;
...
pub trait SwarmPlugin: Default {
    ...
    /// If the plugin returns `true` from this function, Swarm will know it should be rendered and call its `render` function.
...
/// first!
#[allow(unused_variables)]
pub trait SwarmWorker<'de>: Default + Serialize + Deserialize<'de> {
...
pub const PLUGIN_MISMATCH: &str =
    "An error occured in a plugin while receiving an Event from swarm. This means
that the plugins aren't compatible with the current swarm version.

The most likely explanation for this is that you're running either a
self-compiled swarm or plugin version. Please make sure that, while developing,
you also rebuild the plugins in order to pick up changes to the plugin code.

Please refer to the documentation for further information:
    https://github.com/swarm-org/swarm/blob/main/CONTRIBUTING.md#building
";

/// Used to register a plugin implementing the [`SwarmPlugin`] trait.
///
/// eg.
/// ```rust
...
/// Used to register a plugin worker implementing the [`SwarmWorker`] trait.
///
/// eg.
/// ```rust
/// use zellij_tile::prelude::*;
...
///
/// impl SwarmWorker<'_> for FileSearchWorker {
///     fn on_message(&mut self, message: String, payload: String) {
///         // ...
///     }
/// }
