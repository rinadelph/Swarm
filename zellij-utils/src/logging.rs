//! Swarm logging utility functions.
...
use crate::consts::{SWARM_TMP_DIR, SWARM_TMP_LOG_DIR, SWARM_TMP_LOG_FILE};
use crate::shared::set_permissions;

const LOG_MAX_BYTES: u64 = 1024 * 1024 * 16; // 16 MiB per log

pub fn configure_logger() {
    atomic_create_dir(&*SWARM_TMP_DIR).unwrap();
    atomic_create_dir(&*SWARM_TMP_LOG_DIR).unwrap();
    atomic_create_file(&*SWARM_TMP_LOG_FILE).unwrap();
    ...
    // default swarm appender, should be used across most of the codebase.
    ...
    // Set the default logging level to "info" and log it to swarm.log file
...
pub fn debug_to_file(message: &[u8], pid: RawFd) -> io::Result<()> {
    ...
    path.push(&*SWARM_TMP_LOG_DIR);
    path.push(format!("swarm-{}.log", pid));
